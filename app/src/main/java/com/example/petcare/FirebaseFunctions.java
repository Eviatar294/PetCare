        ...
        petRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    if (pet != null) {
                        Log.d("FirebaseFunctions", "Pet found with ID: " + pet.getPetId());
                        Log.d("FirebaseFunctions", "Pet has image: " + (pet.getImageString() != null && !pet.getImageString().isEmpty()));
                        callback.onSuccess(pet);
                    } else {
                        Log.e("FirebaseFunctions", "Failed to parse pet data from snapshot");
                        callback.onFailure("Failed to parse pet data");
                    }
                } else {
                    Log.e("FirebaseFunctions", "Pet not found with ID: " + petId);
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Log.e("FirebaseFunctions", "Error fetching pet: " + error.getMessage());
                callback.onFailure(error.getMessage());
            }
        });
    }
